...
using namespace std;
enum Simulation {MAX_ITER = 100, NUM_PEEK = 1000000, SEED = 100};
enum Code {
...
inline int FP_Decoder::checkPost()
{
	int i = 0;
	BitError = 0;
	double temp = 0;
...
inline int FP_Decoder::max(int x, int y){		
	if(x >= y)
		return x;
	else
		return y;
...
inline int FP_Decoder::sxor(int x, int y){		
	int v1, v2;
	int sum;
	int diff;
	int part1;
