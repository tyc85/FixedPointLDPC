...
% note the index with 0 gives the index in G for generating parity
% it seems to confusing but it's correct=> index_par is the identity
% part after Gaussian elimination
indx_par = find(1- column_flag); % find zero
indx_info = find(column_flag); % find 1
fclose(FID2);
...
fclose(FID);
%%
FID_out = fopen('H_array_p47_r5g_bin.txt', 'w');

fprintf(FID_out, '%d %d', n-m, m);%  # of parities and # of information bits
fprintf(FID_out, '\n');
for i = 1: numel(indx_par)
	fprintf(FID_out, '%d ', indx_par(i));
end
fprintf(FID_out, '\n');

for i = 1: numel(indx_info)
	fprintf(FID_out, '%d ', indx_info(i));
end
fprintf(FID_out, '\n');
for i=1:n-m
	for j = 1:m
